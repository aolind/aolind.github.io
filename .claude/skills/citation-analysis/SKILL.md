...
Normalize each title to a key: lowercase, first 50 chars, alphanumeric only. Within a single PDF, if two rows share a key, sum their cited-by counts into one entry.
...
Report: new PDFs read vs loaded from cache, total snapshots, date range, distinct papers tracked, current citations (h-index, i10-index), output path.
